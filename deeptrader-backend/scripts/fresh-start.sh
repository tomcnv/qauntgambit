#!/bin/bash
#
# DeepTrader Fresh Start Script
# =============================
# Clears ALL trading data (Redis + Database) while preserving configurations.
#
# Usage:
#   ./scripts/fresh-start.sh
#
# What gets DELETED:
#   - All Redis bot:* keys (positions, metrics, orders, etc.)
#   - All paper_trades, paper_positions, paper_orders, paper_balances
#   - All fast_scalper_trades, fast_scalper_positions (live trades)
#   - All decision_traces, replay_snapshots (large data)
#   - All analytics, VaR, equity curves
#   - All backtest data
#   - All alerts and incidents
#
# What gets PRESERVED:
#   - User accounts
#   - Exchange accounts (credentials)
#   - Bot configurations
#   - Profiles and strategies
#   - Market data (candles, order books)
#   - Audit logs
#
...
echo ""
echo -e "${BLUE}╔══════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║           🧹 DeepTrader Fresh Start Script                   ║${NC}"
echo -e "${BLUE}╚══════════════════════════════════════════════════════════════╝${NC}"
echo ""
echo -e "${YELLOW}⚠️  WARNING: This will DELETE ALL trading data!${NC}"
echo ""
echo "   What will be DELETED:"
echo "   ├── All Redis bot:* keys"
echo "   ├── All paper trades, positions, orders, balances"
echo "   ├── All live/fast_scalper trades and positions"
echo "   ├── All decision traces (~5M+ rows, ~5GB)"
echo "   ├── All replay snapshots (~1.5M rows, ~1GB)"
echo "   ├── All analytics, VaR, equity curves"
echo "   ├── All backtest data"
echo "   └── All alerts and incidents"
echo ""
...
echo ""
read -p "Type 'FRESH START' to continue: " confirm
