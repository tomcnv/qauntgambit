#!/usr/bin/env python3
"""Run replay test with detailed logging for rejection analysis."""
...
async def main():
    """Run replay test with detailed logging."""
    # Configure logging to capture rejection details
    logging.basicConfig(
        level=logging.INFO,
        format='%(asctime)s - %(name)s - %(levelname)s - %(message)s',
        handlers=[
            logging.FileHandler('/tmp/replay_detailed.log'),
    ...
    logging.getLogger('quantgambit.observability.logger').setLevel(logging.INFO)
    
    input_path = Path("/tmp/last_12h_full.jsonl")
    output_path = Path("/tmp/replay_results_detailed.json")
    ...
    print(f"Logging to: /tmp/replay_detailed.log")
    ...
    print("Running replay with detailed logging...")
    ...
    print(f"Detailed logs saved to /tmp/replay_detailed.log")
